...
for epoch in range(opt.n_epochs):
    ...
    for i, (imgs, labels) in enumerate(dataloader):
        ...
        g_loss_epo.append(g_loss.item())
        d_loss_epo.append(d_loss.item())

        batches_done = epoch * len(dataloader) + i
        if batches_done % opt.sample_interval == 0:
            sample_image(n_row=10, batches_done=batches_done)

    loss_g.append(np.mean(g_loss_epo))
    loss_d.append(np.mean(d_loss_epo))
...
x = np.arange(1, opt.n_epochs + 1)
l1 = plt.plot(x, loss_g, "b--", label="g_loss")
l1 = plt.plot(x, loss_d, "r--", label="d_loss")
plt.xlabel("epoch")
plt.ylabel("loss")
plt.legend()
...
number = "13845631383409"  # TODO: 写入比赛页面中指定的数字序列（字符串类型）
n_row = len(number)
